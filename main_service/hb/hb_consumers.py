...
def update_hb_consumers_daily(days=0):
    today_date = DateUtil.date2str(DateUtil.getDateBeforeDays(int(days)), '%Y-%m-%d')
    end_date = DateUtil.date2str(DateUtil.getDateAfterDays(1 - int(days)), '%Y-%m-%d')
...
def update_hb_consumers_weekly(days=0):
    start_weekdate, end_weekdate = DateUtil.getThisWeekDate()
...
def update_hb_consumers_monthly(days=0):
    start_monthdate, end_enddate = DateUtil.getThisMonthDate()
...
def update_hb_consumers_quarterly():
    start_quarterdate, end_quarterdate = DateUtil.getLastQuarterDate()
...
def update_hb_newconsumers_daily(days=0):
    yesterday = DateUtil.getBeforeDays(days)
