...
def update_hb_inter_coupon_cost_daily(days=0):
    ...
    pay_cost_in_return_sql = """
        
        select distinct TRADE_TIME s_day,
        sum(case when (AMOUNT_TYPE=2 and PRODUCT='68' and TRADE_CHANNEL not like '%%coupon%%') then amount else 0 end) paycost_in,
        sum(case when (AMOUNT_TYPE=3 and PRODUCT='68' and TRADE_CHANNEL not like '%%coupon%%') then amount else 0 end) paycost_return
        from PAY_COST_INFO 
        where TRADE_TIME>=%s
        and TRADE_TIME<%s
        group by TRADE_TIME
    ...
    pay_cost_in_return = DBCli().pay_cost_cli.query_all(pay_cost_in_return_sql, dto)
    ...
    DBCli().targetdb_cli.batch_insert(insert_pay_cost_in_sql, pay_cost_in_return)
