...
def update_hotel_newconsumers_daily_history():
    ...
    while start_date <= end_date:
        ...
        # query_data = sky_hotel_cli.query_one(hotel_newconsumers_sql["hotel_newconsumers_daily"], dto)
        query_data = sourcedb_cli.query_one(hotel_newconsumers_sql["hotel_newconsumers_daily"], dto)
...
def get_uids_history():
    ...
    """
    query_data = sourcedb_cli.query_all(sql)
...
def update_new_hotel_newconsumers_daily_history():
    ...
    while start_date <= end_date:
        ...
        query_data = sky_hotel_cli.query_all(hotel_newconsumers_sql["hotel_newconsumers_daily"], dto)
        p2p_num = sky_hotel_cli.query_one(hotel_newconsumers_sql["hotel_newconsumers_p2p_daily"], dto)
        for uid in query_data:
