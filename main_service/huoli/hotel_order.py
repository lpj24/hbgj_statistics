...
def update_hotel_orders_daily(days=0):
    ...
    if days > 0:
        start_date = DateUtil.date2str(DateUtil.getDateBeforeDays(3), '%Y-%m-%d')
        end_date = DateUtil.date2str(DateUtil.getDateAfterDays(0), '%Y-%m-%d')
    else:
        end_date = DateUtil.date2str(DateUtil.getDateAfterDays(1), '%Y-%m-%d')
